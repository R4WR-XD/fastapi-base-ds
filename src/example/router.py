...
@router.get("/vehiculos", response_model=schemas.Vehiculo)
...
def read_vehiculo(vehiculo_id: str, db: Session = Depends(get_db)):
    return services.leer_vehiculo(db, vehiculo_id)


@router.put("/vehiculos/{vehiculo_id}", response_model=schemas.Vehiculo)
def update_vehiculo(
    vehiculo_id: str, vehiculo: schemas.VehiculoUpdate, db: Session = Depends(get_db)
):
...
def delete_vehiculo(vehiculo_id: str, db: Session = Depends(get_db)):
